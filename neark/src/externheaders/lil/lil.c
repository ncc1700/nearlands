...
#define malloc core_alloc
#define free core_free
//#define printf(message, ...) kterm_write_printf(INFO, "%s " message, __FUNCTION__, ##__VA_ARGS__)

void printf(char* message, ...){
    kterm_write_printf(INFO, "print called\n");
}

void sprintf(char *restrict, const char *restrict, ...){
...
void* realloc(void* prev, int size){
    return malloc(size);
}
...
{
    return 0;//lil_alloc_double(rand()/(double)RAND_MAX);
}
