...
/**
 * This is a very unefficient memory allocator
 *
 *
 * thing is however it works, so if you don't mind large freemap caching
 * and O(n) allocation and free feel free to use it
 *
 *
 *
 */
...
typedef struct __attribute__((packed)) _FreeMap {
    uint64_t base;
    uint8_t isFree:1;
} FreeMap;
    ...
    for(uint64_t i = 0; i < freeMapAmount; i++){
        ...
        if(freemap[i].isFree == 1){
            ...
            for(int j = 0; j < amount; j++){
                ...
                if(freemap[i].base != (freemap[i + j].base - (j * 4096))){
                    shouldBreak = 1;
                    break;
                }
            }
