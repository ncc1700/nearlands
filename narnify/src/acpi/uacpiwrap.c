...
void uacpi_kernel_log(uacpi_log_level log, const uacpi_char* str){
    if(QolGetStringLength((u8*)str) == 0) return;
...
uacpi_bool uacpi_kernel_wait_for_event(uacpi_handle event, uacpi_u16 time){
    KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI]: %s: UNIMPLEMENTED"), __FUNCTION__);
    return 0;
}
...
uacpi_status uacpi_kernel_handle_firmware_request(uacpi_firmware_request* firmwareReq){
    KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI]: %s: UNIMPLEMENTED"), __FUNCTION__);
    return 0;
}

uacpi_status uacpi_kernel_install_interrupt_handler(
    uacpi_u32 irq, uacpi_interrupt_handler intHandler, uacpi_handle ctx,
...
uacpi_status uacpi_kernel_uninstall_interrupt_handler(
    uacpi_interrupt_handler intHandler, uacpi_handle irq_handle
){
    KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI]: %s: UNIMPLEMENTED"), __FUNCTION__);
    return 0;
}
...
uacpi_handle uacpi_kernel_create_spinlock(void){
    ...
    //KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI]: %s: UNIMPLEMENTED"), __FUNCTION__);
    SpinLock* lock = KeCreateSpinLockHeap();
    if(lock == NULL){
        KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI] !! lock is NULL, returning NULL lock"));
    }
    return lock;
}
...
uacpi_status uacpi_kernel_io_write32(
    uacpi_handle kernel, uacpi_size offset, uacpi_u32 in_value
){
    KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI]: %s: UNIMPLEMENTED"), __FUNCTION__);
    return 0;
}
...
uacpi_status uacpi_kernel_schedule_work(
    uacpi_work_type workType, uacpi_work_handler workHandler, 
    uacpi_handle ctx
){
    KeTermPrint(TERM_STATUS_ERROR, QSTR("[ACPI]: %s: UNIMPLEMENTED"), __FUNCTION__);
    return 0;
