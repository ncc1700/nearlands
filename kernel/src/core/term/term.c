...
#include "external/mini-printf.h"
...
static inline void draw_rect(uint32_t display, uint32_t x, uint32_t y, uint32_t height, uint32_t width, uint64_t color){
...
static inline void term_putchar_anywhere_display(uint32_t display, char c, uint32_t x, uint32_t y, uint32_t size, uint64_t color){
    if (c < 32 || c > 126) {
        return;
    }
...
static inline void term_putchar_anywhere(char c, uint32_t x, uint32_t y, uint32_t size, uint64_t color){
    term_putchar_anywhere_display(0, c, x, y, size, color);
}

static inline void term_write_anywhere_display(uint32_t display, int x, int y, char* s, int size, uint64_t color){
    int c;
    int current_x = x; 
    int current_y = y;
...
static inline void term_write_anywhere(int x, int y, char* s, int size, uint64_t color){
...
static inline void term_write(char* s, uint64_t color){
...
static inline void term_write_no_line(char* s, uint64_t color){
    if(terminal.ypos == 0 && terminal.xpos == 0){
        term_clear();
        terminal.ypos = 10;
        terminal.xpos = 10;
...
volatile atomic_flag term_lock = ATOMIC_FLAG_INIT;


void term_write_printf(status stat, char* format, ...){
    sl_acquire(&term_lock);
    char s[1024];
    va_list args;
