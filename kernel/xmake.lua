...
target("kernel-aarch64-virt-all-drivers")
    set_toolchains("clangaarch64")
    set_filename("kernel.elf")
    set_targetdir("../bin/SYSTEM/")
    add_files("src/*.c", "src/core/*.c", "src/core/**/*.c");
    add_files("src/hal/aarch64-virt/*.c", "src/hal/aarch64-virt/*.asm")
    add_files("src/hal/aarch64-virt/**/*.c", "src/hal/aarch64-virt/**/*.asm")
    add_files("src/drivers/*.c", "src/drivers/**/*.c")
    add_cflags("-target aarch64-none-elf", "-ffreestanding", "-nostdlib", "-fno-exceptions", "-fno-rtti", "-mcmodel=kernel", {force = true})
    add_cxxflags("-target aarch64-none-elf", "-ffreestanding", "-nostdlib", "-fno-exceptions", "-fno-rtti", "-mcmodel=kernel", {force = true})
