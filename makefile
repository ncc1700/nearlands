...
run-amd64:
	...
	-drive file=image.img,format=raw -m 512M -serial mon:stdio -display sdl

run-amd64-debug:
	qemu-system-x86_64 -bios resources/UEFI/amd64/OVMF.fd \
	-device virtio-gpu-pci \
	-drive file=image.img,format=raw -m 96M -serial mon:stdio -s -S -display sdl
...
run-aarch64-debug:
	qemu-system-aarch64 -bios resources/UEFI/aarch64/OVMF.fd \
		...
		-drive if=none,file=image.img,format=raw,id=hd0 \
  		-device virtio-blk-device,drive=hd0 \
		-serial mon:stdio -s -S -display sdl
